...
import common.DLVManager;
import common.DLVMultiHelper;

public class AIController4 extends AIController
{
	private class TreeWorker extends Thread{
		...
		Model model;
		private int dlvToUse;

		TreeWorker(String fatcsFile, CyclicBarrier barrier, Model model, FinalInChoice finalChoice, int dlvToUse) {
			this.factsFile = fatcsFile;
			this.barrier = barrier;
			this.model = model;
			this.finalChoice = finalChoice;
			this.dlvToUse = 1+dlvToUse%7;
		}
		...
		public void run() {
			super.run();
			try {
				calculateChoiceTree(model, factsFile, finalChoice, dlvToUse);
				barrier.await();
	...
	public AIController4()
	{
		super();
		depth = 4;
	}

	public AIController4(int depth)
	{
		super();
		this.depth = depth;
	}
	...
	public boolean update(Player player)
	{
		...
		try {
			...
			for (int m = 0; m < size; m++){
				workers[m] = new TreeWorker("facts"+(m+1)+".dl", barrier, models.get(m), finalChoice, (m+1));
				workers[m].start();
			}
			barrier.await();
	...
	private void calculateChoiceTree(Model startModel, String facts, FinalInChoice finalChoice, int dlvToUse){
		try
		{
			Choise initChoise = getChoiseFromModel(startModel, facts, dlvToUse);
			if (initChoise != null)
			{
					...
					while (!queue.isEmpty())
					{
						...
						{ // Se ho modelli da valutare
							...
							while (iteratorModels.hasNext())
							{
								...
								{
									Choise choiseFromModel = getChoiseFromModel(nextModel, facts, dlvToUse);
	...
	private Choise getChoiseFromModel(Model model, String factsFile, int dlvToUse) throws CloneNotSupportedException, IOException
	{
		...
		DLVMultiHelper dlvHelper = DLVManager.getDLVMultiHelper(dlvToUse);
		...
		if (models != null)
		{
			...
			choise.gameBoard.reorganizeBoardWithDLV(choise.opInRow, choise.opInColumn, dlvHelper);
		}
		return choise;
	}
}
